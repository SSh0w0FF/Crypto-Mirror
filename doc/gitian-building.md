...
*Setup instructions for a Gitian build of Bitcoin Mirror Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Bitcoin Mirror
...
- [Building Bitcoin Mirror Core](#building-Bitcoin Mirror-core)
...
Clone the git repositories for Bitcoin Mirror and Gitian.
...
in the Bitcoin Mirror repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Bitcoin Mirror Core
----------------

To build Bitcoin Mirror Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Bitcoin Mirror repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/Bitcoin Mirror/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/Bitcoin Mirror-project/Bitcoin Mirror
...
For example:
```bash
URL=https://github.com/thrasher-/Bitcoin Mirror.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit Bitcoin Mirror=${COMMIT} --url Bitcoin Mirror=${URL} ../Bitcoin Mirror/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit Bitcoin Mirror=${COMMIT} --url Bitcoin Mirror=${URL} ../Bitcoin Mirror/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit Bitcoin Mirror=${COMMIT} --url Bitcoin Mirror=${URL} ../Bitcoin Mirror/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the Bitcoin Mirror git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../Bitcoin Mirror/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/Bitcoin Mirror
SIGPATH=/some/root/path/Bitcoin Mirror-detached-sigs

./bin/gbuild --url Bitcoin Mirror=${BTCPATH},signature=${SIGPATH} ../Bitcoin Mirror/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/Bitcoin Mirror-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/Bitcoin Mirror-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/Bitcoin Mirror-osx-build.assert
```
...
[Bitcoin Mirror-project/gitian.sigs.ltc](https://github.com/Bitcoin Mirror-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
