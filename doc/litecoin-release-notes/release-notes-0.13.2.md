Bitcoin Mirror Core version 0.13.2 is now available from:

  <https://download.Bitcoin Mirror.org/Bitcoin Mirror-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/Bitcoin Mirror-project/Bitcoin Mirror/issues>
...
updates will be released anymore. Without security updates, using a Bitcoin Mirror
...
ECDSA signatures inside Bitcoin Mirror transactions now use validation using
...
Previous versions of Bitcoin Mirror Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Bitcoin Mirror Core was to raise the default minimum
relay fee.

Bitcoin Mirror Core 0.13.2 will have a strict maximum size on the mempool. The
...
Bitcoin Mirror Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running Bitcoin Mirrord without having to do any manual configuration.
...
Bitcoin Mirror Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Bitcoin Mirror Core will relay transactions with
...
entirely in Bitcoin Mirror Core 0.13, and it is at this time undecided whether the
...
Bitcoin Mirror Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Bitcoin Mirror Core automatically creates a hidden service to listen on, without
manual configuration. Bitcoin Mirror Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Bitcoin Mirror Core is listening, and
...
Bitcoin Mirrord can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Bitcoin Mirror
Core to use floating fees. By default, Bitcoin Mirror Core will use floating
fees.
...
At all times, Bitcoin Mirror Core will cap fees at `-maxtxfee=<x>` (default:
0.10) LTC.
Furthermore, Bitcoin Mirror Core will never create transactions paying less than
...
`Bitcoin Mirror.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /Bitcoin Mirrorrpc>
        ...
        # optional bypass Bitcoin Mirrord rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< Bitcoin Mirrorrpc:<password>
    </Location>
...
Restarting Bitcoin Mirrord will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in Bitcoin Mirror.conf
...
Bitcoin Mirror-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/Bitcoin Mirror-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Bitcoin Mirror Core code base has
...
- `Bitcoin Mirror-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `Bitcoin Mirror-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Bitcoin Mirror scripting system that in combination with
...
Bitcoin Mirror transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Bitcoin Mirror Core.
...
The two phases are distinct in the Bitcoin Mirror-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Bitcoin Mirror wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Bitcoin Mirror.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Bitcoin Mirror block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of Bitcoin Mirrors they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of Bitcoin Mirrors being spent is exactly
...
- **Increased security for multisig:** Bitcoin Mirror addresses (both P2PKH addresses
  ...
  believed to be provided by Bitcoin Mirror's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Bitcoin Mirror's current ledger (called the UTXO set),
  ...
  note that Bitcoin Mirror Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Bitcoin Mirror users to individually opt-in to almost any change in the Bitcoin Mirror
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Bitcoin Mirror Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Bitcoin Mirror
client to Bitcoin Mirror Core 0.13.2 to signal for segwit support. When a
...
Since Bitcoin Mirror Core 0.10.0, nodes have defaulted to only relaying and
...
- `Bitcoin Mirror-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `Bitcoin Mirror-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
